...
const PokeCard = ({ data }) => {
  console.log(data);
  return (
    <div className={`${colors[data.types[0].type.name].dark} aspect-5/3 rounded-2xl`}>
      <div className="flex p-4 w-full h-full relative">
        <div className="w-2/5  absolute md:top-8 md:left-8">
          <h2 className={`capitalize font-bold  ${colors[data.types[0].type.name].text} text-xl md:text-2xl lg:text-3xl xl:text-6xl mb-2`}>
            {data.name}
          </h2>
        ...
        <h3 className={`${colors[data.types[0].type.name].textLight} absolute top-3 right-3 md:top-8 md:right-8 text-sm md:text-lg lg:text-xl xl:text-3xl`}>
          #{data.id}
        </h3>
